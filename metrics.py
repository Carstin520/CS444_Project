...
def calc_metrics(out, target, loss_fn_alex):
	psnr = calc_psnr_np(out, target)
	SSIM = ssim(out, target, win_size=11, data_range=255, channel_axis=-1, gaussian_weights=True)
	LPIPS = calc_lpips(out, target, loss_fn_alex)
	return np.array([psnr, SSIM, LPIPS], dtype=float)
...
if __name__ == '__main__':
	...
	parser.add_argument('--name', type=str, required=True,
			            help='Name of the folder to save models and logs.')	
	parser.add_argument('--dataroot', type=str, default='/content/drive/MyDrive/GOPRO_Large/')
	parser.add_argument('--device', default="0")
	args = parser.parse_args()
	...
	root = '/content/drive/MyDrive/SelfIR/'
	files = [
		root + 'ckpt/' + args.name,
	]
